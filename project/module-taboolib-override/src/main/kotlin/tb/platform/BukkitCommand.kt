...
class BukkitCommand : PlatformCommand {
        ...
        submit(now = true) {
            ...
            try {
                ...
                pluginCommand.setProperty(
                    "permissionMessage",
                    Class
                        .forName("$a$b.adventure.text.Component")
                        .invokeMethod<Any>("text", permission, isStatic = true)
                )
            }
            // 注册权限
            fun registerPermission(permission: String, default: PermissionDefault) {
                if (Bukkit.getPluginManager().getPermission(permission) == null) {
                    try {
                        val p =
                            Permission(permission, org.bukkit.permissions.PermissionDefault.values()[default.ordinal])
            ...
            kotlin.runCatching {
                if (pluginCommand.getProperty<Any>("timings") == null) {
                    ...
                    pluginCommand.setProperty(
                        "timings",
                        timingsManager.invokeMethod("getCommandTiming", plugin.name, pluginCommand, isStatic = true)
                    )
                }
            }
            sync()
    ...
    override fun unknownCommand(sender: ProxyCommandSender, command: String, state: Int) {
        when (state) {
            1 -> sender.cast<CommandSender>().spigot()
                .sendMessage(TranslatableComponent("command.unknown.command").also {
                    it.color = ChatColor.RED
                })

            2 -> sender.cast<CommandSender>().spigot()
                .sendMessage(TranslatableComponent("command.unknown.argument").also {
                    it.color = ChatColor.RED
                })
