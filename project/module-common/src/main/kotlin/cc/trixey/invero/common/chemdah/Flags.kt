...
enum class Flags(val match: (String, String) -> Boolean) {
    ...
    TAG({ name, value ->
        ...
        value.asTags().any { it.name.equals(name, true) }
    }),
    ...
    companion object {
        ...
        fun String.asTags(): Set<Material> {
            return tagsMap.computeIfAbsent(this) {
                ...
                set.addAll(
                    Bukkit.getTag("blocks", NamespacedKey.minecraft(this), Material::class.java)?.values ?: emptySet()
                )
                set.addAll(
                    Bukkit.getTag("items", NamespacedKey.minecraft(this), Material::class.java)?.values ?: emptySet()
                )
                set
            }
        }
